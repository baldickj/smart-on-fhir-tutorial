    ...
    window.extractData = function() {
        ...
        function onReady(smart)  {
            if (smart.hasOwnProperty('patient')) {
                ...
                // Fetch encounters to get encounter IDs
                ...
                $.when(pt, obv, enc).done(function(patient, obv, enc) {
                    ...
                    // Get document references for each encounter
                    var documentPromises = enc.map(function(encounter) {
                    ...
                    $.when.apply($, documentPromises).then(function() {
                        var allDocs = Array.prototype.concat.apply([], arguments);
                        var docTable = $('#documentBody');

                        allDocs.forEach(function(doc) {
                            if (doc.content && doc.content[0] && doc.content[0].attachment) {
                                var docUrl = doc.content[0].attachment.url;
                                var docName = doc.description || 'Document';
                                var docRow = `<tr>
                                    <td>${docName}</td>
                                    <td><button onclick="openDocument('${docUrl}')">Open</button></td>
                                </tr>`;
